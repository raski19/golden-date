...
app.get("/api/synergy", async (req: Request, res: Response) => {
  try {
    // Expect a comma-separated list of IDs: ?userIds=123,456,789
    const { userIds, days = "30" } = req.query;
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const individualResults = users.map((user) => {
        const analysis = calculateScore(user, dayInfo);
        return {
          name: user.name,
          score: analysis.score,
      ...
      // 2. THE WEAKEST LINK CHECK
      // If ANYONE has a "DANGEROUS" verdict (Clash/Breaker), the group fails.
      const hasDanger = individualResults.some(
        (r) => r.verdict === "DANGEROUS",
      );

      // 3. Calculate Group Score
      let groupScore = 0;
      let groupVerdict = "NEUTRAL";
      let cssClass = "neutral";

      if (hasDanger) {
        groupScore = 0;
        groupVerdict = "CONFLICT"; // New Status
        cssClass = "dangerous";
      } else {
        // Average the scores
        const total = individualResults.reduce((sum, r) => sum + r.score, 0);
        groupScore = Math.floor(total / users.length);

        if (groupScore >= 90) {
          groupVerdict = "POWER TEAM";
          cssClass = "golden";
        } else if (groupScore >= 75) {
          groupVerdict = "EXCELLENT";
          cssClass = "excellent";
      ...
      results.push({
        ...
        groupVerdict,
        cssClass,
        breakdown: individualResults, // Show who is dragging the team down!
      });
    }
    ...
    console.error(error);
    res.status(500).json({ error: "Synergy calc failed" });
  }
});
